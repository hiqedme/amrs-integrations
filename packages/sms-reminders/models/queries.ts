...
export async function dailyAppointmentsquery(appointmentDate: string) {
  ...
  consent.patient_sms_consent_provided as consent,
  consent.sms_receive_time as smsReceiveTime,
  ...
  consent.language_preference
              FROM
                ...
                amrs.person_attribute contacts ON (t1.person_id = contacts.person_id
                    ...
                    LEFT JOIN etl.flat_consent consent ON (consent.person_id = t1.person_id)
              WHERE
...
export async function fetchClientsWithPendingDeliveryStatus() {
  let amrsCON = await CM.getConnectionAmrs();
  let sql = `select message_id from etl.sms_delivery_report where delivery_status in("pending")`;
  console.log("Query", sql);
